# README-WAKE — dsh-plus（deepseek-harness-workbench-plugin）

> V1-20260818

## 定位

`projects/coder-agent/dsh-plus/` 是**专门管理 dsh（DeepSeek Harness）插件的目录**，本仓库即该目录下的第一个插件。
...
| 本地路径 | `projects/coder-agent/dsh-plus/`（**独立 git**，不属于上级目录 repo） |
| 默认分支 | `main`（维持纯净，跟随上游） |
